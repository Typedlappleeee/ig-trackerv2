...
- **Concurrency**: a story ≈ 40 real‑time ADB calls (`/shell/execute`), a Reels post = 1 GeeLark RPA task. Running many phones at once can raise transient "shell not ready" responses (busy shell daemon) → some ADB actions fail. Concurrency is user‑controlled via "Téléphones simultanés" (default **Tous**). (Note: no documented GeeLark req/min limit — earlier claims of "~200 req/min" were an unverified project assumption, not from GeeLark docs.)
