...
export function NotificationChannels({ user }: { user: User }) {
  ...
  async function save() {
    ...
    else setTestMsg({ ok: false, text: `Erreur d'enregistrement : ${error.message}` })
  }
  ...
  async function test() {
    ...
    try {
      const r = await fetch('/api/notify', {
        ...
        body: JSON.stringify({
          settings: s,
          subject: '✅ Test ScaleFlow',
          message: 'Si tu reçois ce message, tes notifications sont bien configurées.',
        }),
      })
      const j = await r.json()
      if (j.configured === 0) setTestMsg({ ok: false, text: 'Aucun canal configuré — renseigne au moins Telegram, Discord ou un email.' })
      else if (j.ok)          setTestMsg({ ok: true, text: `Message envoyé sur ${j.configured} canal(aux). Vérifie tes notifications.` })
      else                    setTestMsg({ ok: false, text: `Échec d'envoi. Vérifie tes identifiants. (${JSON.stringify(j.results)})` })
    } catch (e) {
      setTestMsg({ ok: false, text: `Erreur réseau : ${e instanceof Error ? e.message : String(e)}` })
    } finally {
      setTesting(false)
    }
  }

  if (loading) return <div style={{ padding: 20, color: FAINT, fontSize: 13 }}>Chargement…</div>
  ...
  return (
    <div className="sf-card" style={{ padding: '18px 22px' }}>
      <h3 className="text-[11px] font-semibold uppercase mb-1" style={{ letterSpacing: '0.12em', color: 'var(--muted)', borderLeft: '2px solid var(--accent)', paddingLeft: 8 }}>
        Canaux externes {currentOrg ? '(organisation)' : ''}
      </h3>
      <p style={{ fontSize: 12.5, color: FAINT, margin: '8px 0 16px', lineHeight: 1.5 }}>
        Reçois des alertes hors de l'app — même PC éteint. Telegram et Discord marchent tout de suite,
        l'email nécessite une clé Resend côté serveur.
      </p>

      <Field
        label="Discord — URL de webhook"
        hint="Le plus simple : Discord → Paramètres du salon → Intégrations → Webhooks → copier l'URL."
        value={s.discord_webhook ?? ''}
        onChange={v => setS(p => ({ ...p, discord_webhook: v }))}
        placeholder="https://discord.com/api/webhooks/…"
      />
      <Field
        label="Telegram — token du bot"
        hint="Crée un bot via @BotFather sur Telegram, colle le token ici."
      ...
      <Field
        ...
        hint="Envoie un message à ton bot, puis récupère ton chat_id (ex : via @userinfobot)."
      ...
      <Field
        label="Email"
        type="email"
        hint="Nécessite RESEND_API_KEY côté serveur (Vercel + Supabase)."
        value={s.email ?? ''}
      ...
      <div style={{ marginTop: 8, marginBottom: 16 }}>
        <label style={{ display: 'block', fontSize: 11.5, fontWeight: 600, color: MUTED, marginBottom: 8 }}>Quand être alerté</label>
        {EVENT_DEFS.map(ev => {
          ...
          return (
            ...
            >
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: 12.5, fontWeight: 600, color: IVORY }}>{ev.label}</p>
                <p style={{ fontSize: 11, color: FAINT, marginTop: 1 }}>{ev.desc}</p>
              </div>
              <span style={{
