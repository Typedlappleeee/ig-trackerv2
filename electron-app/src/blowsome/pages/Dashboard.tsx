  ...
  return (
    <div>
      ...
      <BlowCard style={{ padding: 28, marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <h1 style={{ margin: '14px 0 6px', fontSize: 34, fontWeight: 900, letterSpacing: '-.035em', textTransform: 'capitalize' }}>
            Bonjour, <Grad style={{ animation: 'blow-shimmer 6s linear infinite' }}>{firstName}</Grad>
          </h1>
          <p style={{ margin: 0, fontSize: 14.5, color: MUTED, maxWidth: 520 }}>
            Ton cockpit VIP — publie, pilote tes flottes et retrouve toute ta banque, sans quitter Blowsome.
          </p>
          <div style={{ display: 'flex', gap: 10, marginTop: 20, flexWrap: 'wrap' }}>
            <BlowButton onClick={onPublish}><Ico d={ICON.send} size={15} /> Publier maintenant</BlowButton>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 24 }}>
        <BlowStat label="Postings en cours" value={running.length} icon={<Ico d={ICON.send} size={16} />} accent="#A855F7" delay={0.05} />
        <BlowStat label="Téléphones" value={phoneCount === null ? '…' : phoneCount} icon={<Ico d={ICON.phone} size={16} />} accent="#EC4899" delay={0.1} />
        <BlowStat label="Vidéos en banque" value={videoCount === null ? '…' : videoCount} icon={<Ico d={ICON.folder} size={16} />} accent="#6366F1" delay={0.15} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
        <BlowCard style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: `1px solid ${HAIR}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13.5, fontWeight: 800 }}>Postings en cours</span>
            <button onClick={() => onGo('posting')} style={{ fontSize: 11.5, color: '#D8B4FE', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700 }}>Tout voir →</button>
          </div>
          {running.length === 0 ? (
            <BlowEmpty title="Rien en cours" hint="Lance une publication — elle apparaîtra ici en direct." icon={<Ico d={ICON.send} size={20} />} />
          ) : (
            running.slice(0, 4).map((r, i) => {
              ...
              return (
                <div key={r.id} style={{ padding: '13px 20px', borderBottom: i < Math.min(running.length, 4) - 1 ? `1px solid ${HAIR}` : 'none' }}>
                  ...
                  <p style={{ margin: '6px 0 0', fontSize: 11, color: FAINT }}>{r.done}/{r.total} comptes · {pct}%</p>
                </div>
              )
            })
          )}
        ...
        <BlowCard style={{ padding: 22, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 12, position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ margin: 0, fontSize: 18, fontWeight: 800, lineHeight: 1.35, position: 'relative' }}>
            Publie sur toutes tes flottes <Grad>en un clic</Grad>.
          </p>
          <p style={{ margin: 0, fontSize: 13, color: MUTED, lineHeight: 1.5, position: 'relative' }}>
            Ouvre le posting ScaleFlow directement ici, sans quitter ton espace Blowsome.
          </p>
          <BlowButton onClick={onPublish} style={{ alignSelf: 'flex-start', marginTop: 4 }}><Ico d={ICON.send} size={15} /> Publier maintenant</BlowButton>
        </BlowCard>
      </div>
    </div>
  )
