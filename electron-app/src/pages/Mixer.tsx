...
function SectionHeader({ children, count }: { children: React.ReactNode; count?: number }) {
  return (
    <div className="flex items-center justify-between mb-2">
      ...
      {count !== undefined && <span className="text-xs text-muted">{count} sélectionnés</span>}
    </div>
  )
}
...
export function Mixer({ user }: MixerProps) {
  ...
  const toggleVideo = (id: string) =>
    setSelVideos(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n })

  const toggleCaption = (id: string) =>
    setSelCaptions(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n })
  ...
  const buildJobs = (): MixJob[] => {
    ...
    if (mode === 'all') {
      return vids.flatMap(v => caps.map(c => ({
        id: `${v.id}-${c.id}`,
        videoItem: v,
        caption: c,
    ...
    // random: each video gets one caption (cycle through captions)
    return vids.map((v, i) => ({
  ...
  const startMix = async () => {
    ...
    const processJob = async (job: MixJob) => {
      ...
      try {
        ...
        // Increment used_count on the caption
        supabase.from('caption_bank')
          .update({ used_count: (job.caption.used_count ?? 0) + 1 })
          .eq('id', job.caption.id)
          .then(() => {})
      } catch (e: any) {
    ...
    for (let i = 0; i < queue.length; i += CONCURRENCY) {
      await Promise.all(queue.slice(i, i + CONCURRENCY).map(processJob))
    }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex flex-col w-1/3 min-w-0 p-4 gap-4 overflow-y-auto">
          ...
          {canStart && (
            <div className="bg-accent/5 border border-accent/20 rounded-lg p-3 text-xs text-accent">
              {mode === 'random'
                ? `${selVideos.size} vidéo${selVideos.size > 1 ? 's' : ''} × 1 caption aléatoire = ${selVideos.size} résultat${selVideos.size > 1 ? 's' : ''}`
      ...
      {error && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-red-900/90 text-red-200 text-xs px-4 py-2 rounded-xl">
          {error}
        </div>
      )}
    </div>
