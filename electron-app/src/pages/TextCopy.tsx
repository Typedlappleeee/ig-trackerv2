...
// 5 vertical positions evenly spread top → bottom
const POSITIONS = [
...
export function TextCopy({ user: _user }: { user: User }) {
  ...
  const [copies,    setCopies]    = useState(3)   // how many position variants per video
  ...
  async function pickVideos() {
    if (isWeb) {
      // web: trigger file picker multiple times or use input[multiple]
      ...
      const path = await (window.electronAPI as any).pickVideoFile?.()
  ...
  async function generate() {
    ...
    if (videos.length === 0) { toast.show({ title: 'Ajoute au moins une vidéo', kind: 'error' }); return }

    // Build job list: each video × chosen number of positions
    const positions = POSITIONS.slice(0, Math.min(copies, POSITIONS.length))
    const jobList: Job[] = []
    let idx = 0
    ...
    for (const job of jobList) {
      ...
      try {
        const yFrac = positions[job.posIdx].yFrac
        ...
        outputPath = tmp?.ok ? tmp.path : `web-output-${Date.now()}.mp4`
        ...
        if (!res.ok) {
          updateJob(job.id, { status: 'error', error: res.error ?? 'Erreur FFmpeg' })
        } else {
          updateJob(job.id, { status: 'done', outputPath: res.outputPath ?? outputPath })
        }
      } catch (e) {
    ...
    <div className="flex flex-col gap-6 p-6 max-w-3xl mx-auto">
      ...
      <section className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.04)' }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            <button onClick={pickVideos} className="px-3 py-1.5 rounded-lg text-sm font-semibold"
              style={{ background: 'rgba(139,92,246,0.2)', color: '#a78bfa' }}>
              + Ajouter
            </button>
            {videos.length > 0 && (
              <button onClick={() => setVideos([])} className="px-3 py-1.5 rounded-lg text-sm font-semibold"
        ...
        {videos.length === 0 ? (
          <p className="text-sm text-white/30 text-center py-4">Aucune vidéo ajoutée</p>
        ) : (
      ...
      <section className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.04)' }}>
        ...
        <textarea
          ...
          placeholder="Entre le texte à afficher sur la vidéo…"
          rows={3}
      ...
      <section className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: 'rgba(255,255,255,0.04)' }}>
        ...
        <p className="text-xs text-white/30">
          {videos.length} vidéo{videos.length > 1 ? 's' : ''} × {copies} position{copies > 1 ? 's' : ''} = <strong className="text-white/50">{videos.length * copies} fichier{videos.length * copies > 1 ? 's' : ''}</strong>
        </p>
      </section>
      ...
      <div className="flex gap-3">
        {!running ? (
          <button onClick={generate} disabled={!text.trim() || videos.length === 0}
              ...
              return (
                <div key={job.id} className="flex items-center justify-between gap-3 text-sm px-3 py-2 rounded-lg"
                  ...
                  {job.status === 'done' && job.outputPath && (
                    <a href={job.outputPath} download={`textcopy_${String(job.id + 1).padStart(3, '0')}.mp4`}
