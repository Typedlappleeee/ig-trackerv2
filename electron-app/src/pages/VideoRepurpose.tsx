        ...
        <div className="anim-stagger" style={{ width: 278, flexShrink: 0, borderRight: '1px solid var(--border)', overflowY: 'auto', background: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div style={{ padding: '0 14px' }}>
            ...
            <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--text-4)', marginTop: 2 }}>
              {sources.length > 1
                ? <span><span style={{ color: '#22d3ee', fontWeight: 700 }}>{count}</span> /vidéo · <span style={{ color: '#a78bfa', fontWeight: 700 }}>{totalJobs}</span> total</span>
              ...
              {(isWeb || saveToBank) && (
                <button
                  ...
                  style={{
                    ...
                    background: bankFolder ? 'rgba(139,92,246,0.1)' : 'var(--surface-2)',
                    border: `1px solid ${bankFolder ? 'rgba(139,92,246,0.35)' : 'var(--border)'}`,
                    color: bankFolder ? '#c4b5fd' : 'var(--text-3)', textAlign: 'left',
                  }}>
          ...
          {jobs.length === 0 ? (
            <div className="sf-empty anim-stagger" style={{ height: '100%' }}>
              ...
              <div style={{ position: 'relative', width: 76, height: 76, margin: '0 auto 8px' }}>
                ...
                <div style={{ position: 'absolute', inset: -6, borderRadius: '50%', border: '1px dashed rgba(139,92,246,0.12)', animation: 'spin 20s linear infinite reverse' }} />
            ...
            <div className="sf-modal-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                <div style={{ width: 30, height: 30, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.2)' }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
                </div>
              ...
              <div style={{ maxHeight: 260, overflowY: 'auto', borderTop: '1px solid var(--border)' }}>
                {bankFolders.map(f => (
                  <button key={f}
                    ...
                    className="cursor-pointer"
                    style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 20px', textAlign: 'left', background: f === bankFolder ? 'rgba(139,92,246,0.1)' : 'transparent', border: 'none', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.15s' }}>
                    ...
                    <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: f === bankFolder ? '#c4b5fd' : 'var(--text-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f}</span>
                    {f === bankFolder && <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2.5" strokeLinecap="round"><path d="M20 6L9 17l-5-5"/></svg>}
                  </button>
                ))}
              </div>
            )}
