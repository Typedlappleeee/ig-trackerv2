...
async function _postInstagramStoryInner(
  ...
  // d'Instagram et la langue → on privilégie la BARRE DE RECHERCHE (présente sur
  // toutes les versions récentes) plutôt qu'un tap direct fragile sur la grille
  // (qui a déjà cliqué « Mention » par erreur). Repli : tap direct sur « LINK ».
  ...
  // 4b. Repli : tap direct sur « LINK » dans la grille (barre absente ou recherche
  //     infructueuse).
  if (!linkTapped) {
    const linkSticker =
