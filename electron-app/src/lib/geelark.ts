...
// ── Instagram Reel via ADB direct download (bypasses GeeLark S3) ─────────────
// Same pattern as postInstagramStory: the phone downloads the video itself via
// curl/wget, then we drive the Instagram Reel creation UI via UIAutomator.
// This avoids the GeeLark S3 PUT flow which silently stores corrupted objects.
export interface ReelsConfig {
  videoUrl: string   // public/signed URL of the MP4 to download
  caption?: string   // optional caption
  dryRun?: boolean   // run all steps but stop before publishing
}

export async function postInstagramReels(
  bearer: string,
  phoneId: string,
  config: ReelsConfig,
  log: (m: string) => void,
  ...
  return withPhoneAutoStop(bearer, phoneId, 10 * 60_000, '10min', log,
    () => _postInstagramReelsInner(bearer, phoneId, config, log))
}

async function _postInstagramReelsInner(
  bearer: string,
  phoneId: string,
  config: ReelsConfig,
  log: (m: string) => void,
  ...
  // ── 1. Download video directly on the phone ────────────────────────────────
  log('🎬 Téléchargement de la vidéo sur le téléphone…')
  const vidPath = '/sdcard/DCIM/Camera/sf_reel.mp4'
  const urlEsc = config.videoUrl.replace(/'/g, `'\\''`)
  await shellExec(bearer, phoneId,
    `mkdir -p /sdcard/DCIM/Camera; ` +
    `curl -L -s --max-time 120 -o '${vidPath}' '${urlEsc}' 2>/dev/null || ` +
  ...
  const videoSize = parseInt(ck.output.trim().split(/\s+/)[0] ?? '0', 10) || 0
  log(`   📎 Vidéo téléchargée: ${(videoSize / 1_000_000).toFixed(1)} Mo`)
  if (videoSize < 10_000) {
  ...
  for (let step = 0; step < 4; step++) {
    xml = await dumpXml(bearer, phoneId)
    // Stop early if we're already on the caption/share screen
    const onShareScreen =
      findByText(xml, 'Share', 'Partager', 'Post', 'Publier') ??
      findByResourceId(xml, 'share_button', 'clips_publish_button', 'publish_button')
    if (onShareScreen) break

    const nextBtn =
      findByText(xml, 'Next', 'Suivant', 'Continue', 'Continuer', '>>', '›') ??
      findByResourceId(xml, 'next_button', 'action_next', 'clips_next_button', 'creation_next',
        'share_flow_action', 'creation_flow_next')
    if (!nextBtn) {
      log(`   ↳ Étape ${step + 1}: bouton Suivant non trouvé — on continue`)
      break
    }
    log(`   ↳ Étape ${step + 1}: Suivant`)
    await shellExec(bearer, phoneId, `input tap ${nextBtn[0]} ${nextBtn[1]}`)
    await sleep(3500)
    await dismissPermissionDialog()
  }
  ...
  if (config.caption?.trim()) {
    log('📝 Saisie de la légende…')
    xml = await dumpXml(bearer, phoneId)
    const captionField =
      findByResourceId(xml, 'caption_text', 'caption_edit_text', 'description', 'share_text',
        'clips_caption', 'caption_input') ??
      findByText(xml, 'Write a caption…', 'Rédiger une légende…', 'Add a caption…',
        'Caption', 'Légende', 'Description') ??
      findByTextPartial(xml, 'caption', 'légende', 'write a', 'add a caption')
    const captionPt = captionField ?? [cx, Math.floor(sh * 0.35)] as [number, number]
    await shellExec(bearer, phoneId, `input tap ${captionPt[0]} ${captionPt[1]}`)
    await sleep(1200)
    const escaped = escapeForInputText(config.caption.trim())
    await shellExec(bearer, phoneId, `input text "${escaped}"`)
    await sleep(800)
    // Dismiss keyboard
    await shellExec(bearer, phoneId, 'input keyevent 4')
  ...
  // ── 8. Publish ─────────────────────────────────────────────────────────────
  log('🚀 Publication du Reel…')
  xml = await dumpXml(bearer, phoneId)
  const sharePt =
    findByText(xml, 'Share', 'Partager', 'Post', 'Publier', 'Publish', 'Publish reel', 'Publier le réel') ??
    findByResourceId(xml, 'share_button', 'clips_publish_button', 'publish_button', 'action_share')
  if (sharePt) {
    await shellExec(bearer, phoneId, `input tap ${sharePt[0]} ${sharePt[1]}`)
  } else {
    log('   ⚠️ Bouton Share non trouvé — tap haut-droite')
    await shellExec(bearer, phoneId, `input tap ${Math.floor(sw * 0.88)} ${Math.floor(sh * 0.055)}`)
  }

  // Wait for upload + IG processing (video upload can take 30-90s)
  log('⏳ Attente upload vidéo Instagram…')
  await sleep(8000)

  // Poll until we leave the editor (max 2 min)
  const deadline = Date.now() + 2 * 60_000
  while (Date.now() < deadline) {
    const checkXml = await dumpXml(bearer, phoneId)
    const stillEditing = /caption_edit_text|caption_text|clips_next_button|clips_publish_button|publish_button/.test(checkXml)
    if (!stillEditing) {
      log('✅ Reel publié !')
      return { ok: true }
    }
  ...
  // One last check
  const finalXml = await dumpXml(bearer, phoneId)
  if (/caption_edit_text|clips_publish_button/.test(finalXml)) {
    log('   ⚠️ L\'éditeur semble encore ouvert après 2 min — vérifie manuellement.')
    return { ok: false, error: 'Publication non confirmée (éditeur encore visible après 2 min)' }
  }
