...
export function buildWebAPI() {
  ...
  return {
    ...
    async runFfmpegMixOverlay(opts: { sourcePath: string; caption: string; position: 'top' | 'middle' | 'bottom' | 'custom'; fontSize: number; fontColor: string; posX?: number; posY?: number; gpsSpoof?: boolean; gpsCity?: string; preset?: string; dateDays?: number; audioStoragePath?: string; captionStyle?: 'outline' | 'snapchat'; captionPng?: string; captionH?: number }) {
      try {
        ...
        const r = await fetch('/api/mix-overlay', {
          ...
          body:    JSON.stringify({
            ...
            captionPng:       opts.captionPng,
            captionH:         opts.captionH,
            posX:             opts.posX,
            posY:             opts.posY,
            gpsSpoof:         opts.gpsSpoof,
            gpsCity:          opts.gpsCity,
            preset:           opts.preset,
            dateDays:         opts.dateDays,
